param(
    ...
    [string]$ModelDir = "E:\CamXAll\ZEMO\uniappx\ZeMo-finance\model\screen_omni_ms",
    [string]$OnnxName = "screen_omni_intent.onnx",
    [string]$OutputName = "screen_omni_intent",
    [int]$FeatureDim = 1024
)
...
if (!(Test-Path -LiteralPath $onnxFile)) {
    throw "ONNX file not found: $onnxFile. Run scripts\train-screen-omni-ms-intent.py first."
}
...
& $converter `
    ...
    --inputShape="features:1,1,1,$FeatureDim" `
...
$staticDir = "E:\CamXAll\ZEMO\uniappx\ZeMo-finance\static\models\screen_omni_ms"
New-Item -ItemType Directory -Force -Path $staticDir | Out-Null
Copy-Item -LiteralPath $msFile -Destination (Join-Path $staticDir "screen_omni_intent.ms") -Force
...
Write-Host "MindSpore Lite screen intent model generated: $msFile"
